    ...
    function(err, docs) {
        if (err) {
         ...
         deferred.reject(err);
          ...
          deferred.resolve({
              names: names
              ja: console.log(names[0].displayName);
              ja: console.log(names[1].displayName);
           ames: res.render('index', { title: 'Search Results', page: 'searchResult', name: names, displayName: AuditorDisplayName(req)  })
         }); // return all names
